...
int main(int argc, char** argv) {
  ...
  const auto observationSpaceArg = magic_enum::enum_cast<MlirObservationSpace>(argv[1]);
  CHECK(observationSpaceArg.has_value())
      << fmt::format("Invalid observation space name: {}", argv[1]);
  const MlirObservationSpace observationSpace = observationSpaceArg.value();
  ...
  std::unique_ptr<::mlir_service::Benchmark> benchmark;
  {
    const auto status = benchmarkFactory.getBenchmark(benchmarkMessage, &benchmark);
    CHECK(status.ok()) << "Failed to get benchmark while computing observation : " << status.error_message();
  }
